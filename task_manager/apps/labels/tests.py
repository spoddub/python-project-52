...
class StatusesTest(TestCase):
    ...
    def setUp(self):
        user = get_user_model()
        user.objects.create_user(username=self.testuser_username, password=self.testuser_password)
        self.client.login(username=self.testuser_username, password=self.testuser_password)
    ...
    def test_labels_delete(self):
        ...
        delete_confirm_message = text_constants.DELETE_CONFIRM % {"name": label.name}
